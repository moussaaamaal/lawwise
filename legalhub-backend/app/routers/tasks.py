...
async def create_task(body: CreateTaskRequest, current_user=Depends(get_lawyer)):
...
async def update_task_status(task_id: str, body: UpdateTaskStatusRequest, current_user=Depends(get_lawyer)):
    result = (
        supabase.table("task")
...
async def update_task(task_id: str, body: UpdateTaskRequest, current_user=Depends(get_lawyer)):
    data = body.model_dump(exclude_none=True)
    if not data:
...
async def delete_task(task_id: str, current_user=Depends(get_lawyer)):
...
async def delete_note(note_id: str, current_user=Depends(get_lawyer)):
